...
- (void)deleteDeviceId :(NSString*)_deviceId{
    [self Save_String_Parameter:@"" :_deviceId];
    NSMutableArray *_oldDeviceInfo=[self Get_Paths:_deviceIdKey];
    int index=[self FindId:_oldDeviceInfo :_deviceId];
    if (index!=-1) {
        NSMutableArray *InfoArray = [[NSMutableArray alloc] init];
...
- (NSString *)getDeviceNameById:(NSString *)_deviceId{
    NSString *name=[self Get_String_Parameter :_deviceId];
    if ((name!=nil)&&([name isEqualToString:@""]==NO)) {
        ...
        if (range.location != NSNotFound) {
            ...
            if (range1.location != NSNotFound){
                ...
                name=[NSString stringWithFormat:@"WisCore_%@",name];
            }
            else{
                name=_deviceId;
            }
...
//    }
    ...
    [self Save_String_Parameter:_deviceName :_deviceId];
}
...
- (void)Save_String_Parameter:(NSString *)str :(NSString *)key
{
    NSUserDefaults *defaults =[NSUserDefaults standardUserDefaults];
...
- (NSString *)Get_String_Parameter:(NSString *)key
{
    NSUserDefaults *defaults=[NSUserDefaults standardUserDefaults];
